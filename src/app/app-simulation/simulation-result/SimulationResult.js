...
const SimulationResult = () => {
    ...
    const drawPoints = (simResult, index) => {
        ...
        simResult.sim[index].frames[simResult.sim[index].frames.length - 1].clusters.forEach(cluster => {
    ...
    const fetchSimulationResult = async () => {
        try{
            const res = await axios.get('http://localhost:8888/sim', {
                crossDomain: true,
                headers:{
        ...
        <div className='w-100 px-5'>
            ...
            {simulationResult ?
                (
                        <div>
                            ...
                            <div>Clusters: {simulationResult.sim[simIndex].frames[simulationResult.sim[simIndex].frames.length - 1].clusters.length}</div>
                            <div>Active Nodes: {getActiveNodes(simulationResult.sim[simIndex].frames[simulationResult.sim[simIndex].frames.length - 1].clusters)}</div>
                        </div>
                ) : <div>Loading...</div>}
            ...
            <div className='mr-5'>
                ...
                <div className='d-flex ml-5 mt-5'>
                    <Button onClick={() => roundNavigate(false)} color='success'>Previous Round</Button>
                    <Button onClick={() => roundNavigate(true)} color='success ml-3'>Next Round</Button>
                </div>
            </div>
